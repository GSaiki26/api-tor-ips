...
class API {
  ...
  async Ban_IP(ip) {
    // Chame a função que será usada para fazer uma querry ao Db.
    const code = await Ban_IP(ip).catch((err) => {
      // Em caso de erro, durante a promise:
...
async function Get_IPListFromDanMe(ip) {
  // Tratar os ip do site: https://www.dan.me.uk/torlist/
  ...
  await axios
    .get(url[0])
    .then(async (res) => {
      // Faça a request para o site.
      console.log(
        "Get_IPListFromDanMe(): Request do Danme Aceita com sucesso!"
      );
    ...
    .catch(async (err) => {
      ...
      try {
        ...
        // Em caso de erro, será retornado a lista completamente vazia à função que o chamou.
        console.log(
          "A lista de IPs do Site https://www.dan.me.uk/torlist/ nao esta baixada e foi limitada!"
        );
        return ip;
      }
    });
  ...
  for (let i = 0; i < threat.length; i++) {
    // Um loop para cada elemento dentro da Array.
...
async function Get_IPListFromOnionoo(ip) {
  ...
  // Code
  try {
    // Caso haja erro durante a request.
...
async function Get_AllIPs() {
  ...
  // Data
  let ip = {
    // Aqui será feito a checagem de 30 minutos para se fazer a request.
    list: [
...
async function Get_IPs() {
  // Função que retornará a lista de Ips censurada.
  ...
  // Code
  if (querry[0][0] == undefined) {
    // Consiga a lista de Objetos que contem os Ips
    console.log(
      "Não existe qualquer IP banido! Retornando o resultado de Get_AllIPs()..."
    );
    return result;
  } // Caso querry retorne valores:
  for (let i = 0; i < ip.list.length; i++) {
    // Loop para cada elemento da Array.
    querry[0][0].forEach((valueQuerry) => {
      // Loop para cada Ip banido.
      let values;
      ...
      // Checar primeiro IP
      if (values[0] == valueQuerry.ip) {
        // Caso o primeiro valor seja igual o ip banido.
...
async function Ban_IP(ip) {
  // Função que servirá para banir ip do site.
  console.log(`Iniciando Ban_IP(ip:${ip})...`);
  // Data
  let code; // Variavel definida para se conseguir o código de retorno final.
  // Code
  if (!(ip.length >= 11 && ip.length <= 16) || ip.length == 40) {
    // Essa é uma limitação da quantidade de caracteres do ip recebido.
    code = 400;
    return code;
  }
  await db
    .Db_Execute(`INSERT INTO Banned_IPs VALUES ('${ip}');`)
    .then((result) => {
